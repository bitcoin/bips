...
- The receiver [initiates a Payjoin Session](#session-initiation)
  by sharing a [Payjoin URI](#payjoin-uri) including its directory mailbox to
  get messages.
- The sender  [proceeds by posting a message](#sender-original-psbt-messaging)
  containing a fully signed fallback transaction, known as the *Original PSBT*
  to the mailbox.
- After getting this message, the receiver
  [proceeds by posting a message](#receiver-proposal-psbt-messaging)
  containing a *Proposal PSBT* based on the Original PSBT including receiver
  contributions.
- The sender proceeds by getting this message containing the
  Proposal PSBT, [signing and broadcasting](#sender-signing-and-broadcast) it.

Either party may choose to broadcast the
fallback transaction described by the Original PSBT instead of proceeding
at any time.

The sender and receiver may experience network interruption and proceed with
the protocol by polling since their messages are buffered in the
directory.
...
Since version 1 payloads are neither encrypted nor authenticated, so
