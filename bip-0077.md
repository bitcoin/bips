...
Payjoin protocol described in [BIP 78](bip-0078.mediawiki),
...
URI](#payjoin-uri). The *sender* responds with a fully signed
...
HPKE](#secp256k1-hybrid-public-key-encryption)).

A [*short ID*](#short-id) is derived as a truncated hash of
...
URI](#payjoin-uri) which the receiver shares with the sender
out of band. The `pj=` query parameter contains the mailbox endpoint URL
with additional [receiver fragment
parameters](#receiver-fragment-parameters) that encode the
receiver key and the Payjoin Directory's [OHTTP Key
Configuration](https://www.ietf.org/rfc/_rfc9458.html#name-key-configuration).
...
- Out of band, the receiver shares a bitcoin URI with the sender,
  ...
  short ID](#short-id), bech32-encoded (without a checksum or
  HRP) followed by the [receiver fragment
  parameters](#receiver-fragment-parameters).
...
- The sender's *reply key*, the *Original PSBT*, and optional sender
  ...
  configuration. This is [POSTed](#sender-messaging) to the
...
- Once the receiver is online, it sends `GET` requests [to the receiver
  mailbox](#receiver-messaging) requests to the mailbox. The
...
- The sender validates the *Proposal PSBT* according to [the sender
  checklist](#senders-proposal-psbt-checklist), signs its
...
[checklist](#receivers-original-psbt-checklist).
...
OHTTP [encapsulated](#client-directory-interactions)
