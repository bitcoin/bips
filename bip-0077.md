...
the Payjoin protocol described in [BIP 78](bip-0078.mediawiki). An untrusted
...
However, BIP 78's requirements have proven to be an
...
- The receiver [initiates a Payjoin Session](#session-initiation)
  ...
  epehemeral mailbox hosted on the directory, where it can receive a message
  from the sender
...
mutually exclusive and and only one can be confirmed.
...
Senders which support BIP 78 but not this proposal may POST messages directly to
...
- Add inputs at at random indices.
...
[BIP 78](https://github.com/bitcoin/bips/blob/master/bip-0078._mediawiki#attack-vectors),
