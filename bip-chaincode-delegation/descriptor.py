...
def parse_sortedmulti_descriptor(data: Mapping[str, object]) -> SortedMultiDescriptorTemplate:
    threshold_raw = data.get("threshold")
    if threshold_raw is None:
        raise AssertionError("descriptor threshold is required")
    threshold = int(threshold_raw)
    return SortedMultiDescriptorTemplate(threshold=threshold)
