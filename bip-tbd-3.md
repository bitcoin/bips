...
output instead, should that ever become necessary. The author [believes][ML remaining concerns] the
...
The coinbase transaction is usually crafted by mining pool software. To the best of the author's
knowledge, there does not exist an open source reference broadly in use today for such software.
When this proposal is slated for activation, mining pools are encouraged to update their software to
craft coinbase transactions that are forward-compatible with the changes proposed in this BIP.
...
Test vectors for this rule are available in [`coinbases.json`](./bip-tbd-3/test_vectors/coinbases.json).
...
Bitcoin Core but also of all other implementations the author is aware of.
