...
BIPs are improvement proposals for Bitcoin[^capitalization] that cover the range of interests of the community. The main topic is information and technologies that support and expand the utility of the bitcoin
...
the brief overview provided per the BIP statuses (see [Workflow](#workflow) below) to the audience.
There is no formal or informal decision body that governs Bitcoin development or decides acceptance[^acceptance] of BIPs.
...
the novelty and viability of the idea should be tested by posting about it to the [Bitcoin Development Mailing
...
Authors should avoid showing up out of the blue with a BIP draft.
...
tests whether it is of interest to more people besides the authors.
The authors should work on drafting a BIP only after establishing that the idea may be of interest
...
Specification has been put through its paces and changes to the BIP have stopped.
...
progress, sees accumulated feedback unaddressed, or otherwise appears stalled for a year, anyone may move the BIP
...
[^acceptance]: **What is meant by acceptance?**  
    ...
    instead intends to leave the evaluation of BIPs to the audience.
[^adoption]: **Why does the BIPs repository no longer track adoption?**  
    In the past, some BIPs maintained lists of projects that had implemented the BIP. These lists generated a lot of
    noise to subscribers of the repository, but often listed implementations of questionable quality while quickly
    growing outdated, therefore providing little value. The repository does not track which projects have implemented
    BIPs anymore. Instead, it is recommended that projects publish a list of the BIPs they implement.
...
[^proposes-to-replace]: **Why was "Replaces" retained instead of changing it to "Proposes-to-Replace"?**  
    ...
    author document, but depending on its progress through the Workflow, it may be meanwhile co-owned by community. Who gets to decide
    ...
    determining the content of the new BIP. The authors of the new BIP can unilaterally recommend that it should be
