...
the restriction on the timestamp of the first block of a difficulty adjustment period introduced in
this BIP. Bitcoin Core version [32.0][Core 32.0] and later will also not generate a block template
that violates the restriction on the timestamp of the last block of a difficulty adjustment period
introduced in this BIP. Although it would be extremely unlikely (for the first restriction due to
the grace period used in this proposal, and for the second because the duration of a difficulty
adjustment period should never be negative), miners should use the `curtime` or `mintime` field
from the `getblocktemplate` result of a version which accounts for both restrictions for their
block's timestamp to make sure they always create blocks valid according to this proposal.
Note this is not a new requirement: using a timestamp lower than the `mintime` field from the
`getblocktemplate` result already leads to creating an invalid block.
...
* __1.0.1__ (2026-09-18):
    * Clarify which Bitcoin Core versions account for each of the two timestamp restrictions when
...
[Core 32.0]: https://bitcoincore.org/en/releases/32.0
