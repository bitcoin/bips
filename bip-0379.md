...
Text files have one vector per line, no comments. `*.txt` is space separated,
`*.tsv` tab separated. Expressions contain no whitespace. A key argument is a letter
...
| `opcodes.txt` | 18,185 | `<expr> <type> <ops>` | as above, plus the max ops of a satisfaction, i.e. the quantity the 201-op limit bounds |
...
| `redeem.json` | 46 cases | JSON | optional satisfaction vectors: `{miniscript, valid, can_sign_N, …}` plus an `identifiers` map of keys, preimages and hashes |
...
There is a Golang based minimal harness available that executes all test vectors against
...
The generated static-analysis corpora are dominated by `multi`, `sha256`, `after`,
`andor` and `or_d`; `older()`, `ripemd160()`, `hash256()`, `hash160()` and `and_n()` occur
only in the 42 hand-written lines, never in composition.

The following cases are still missing from the static-analysis corpora:
