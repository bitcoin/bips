...
These three descriptions only hold for expressions that meet the malleability requirements of the
type system, recursively. The type rules assign the modifiers to every expression, but where those
requirements are not met the values need not match the descriptions above; such an expression cannot
be non-malleably satisfied in any case, and neither can any expression containing it.
