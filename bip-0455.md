...
A client supporting the `bspent` message MUST include coins created _before_ the `cutoff` field in `getbspent` requests. A
...
11 GB reduction in bandwidth is achieved. `CompactSize`, which is commonly used in P2P messages to describe collection
lengths, was selected over `VARINT`, which is used internally within Bitcoin Core to represent variable length integers.
The application of `VARINT` as opposed to `CompactSize` offers a further reduction of 4 GB, however the `VARINT`
...
clients. With respect to reconstructable script, utilizing this format results in a savings of around 12 GB. The scheme
is loss-less, and may be upgradable by appending script variants. For reference, the naive encoding of block spent coins
is 118 GB as of block 930,000[^2][^3][^4].
...
very modest savings in bandwidth, around 30 MB. As such, this technique is omitted for implementation simplicity.
