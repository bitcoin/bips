```
  BIP: 451
  Layer: Applications
  Title: Dust Attack UTXO Disposal Protocol
...
- SHOULD try to batch a dust input with an unconfirmed disposal tx if one exists, see "Batching" below.
...
- **No urgency**: There is generally no urgency to disposing of dust attack UTXOs, they can be locked to prevent accidentally spending them before they are confirmed and safely disposed of.
