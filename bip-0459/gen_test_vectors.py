#!/usr/bin/env python3
"""
Generate test vectors for BIP 459 DahLIAS fully aggregated signatures
for secp256k1.
...
def gen_tweak_vectors(f):
    writer = csv.writer(f)
    writer.writerow((
        "index", "internal_secret_key", "tweak", "is_xonly", "expected_result",
    ...
    def row(idx, sk, t, is_xonly, comment):
        ...
        writer.writerow((
            ...
            "TRUE" if is_xonly else "FALSE", "TRUE",
    ...
    def error_row(idx, sk, t, is_xonly, comment):
        for tweak_fn, key in ((TweakSK, sk), (TweakPK, sk * G)):
            try:
                ...
                continue
            raise RuntimeError(f"{tweak_fn.__name__} unexpectedly succeeded: {comment}")
        writer.writerow((
            idx, sk.to_bytes().hex().upper(), t.to_bytes().hex().upper(),
            "TRUE" if is_xonly else "FALSE", "FALSE", "", "",
            comment,
        ))
